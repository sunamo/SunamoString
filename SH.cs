...
", AllStrings.comma);
    ...
    public static string WithoutSpecialChars(string v, params char[] over)
    {
        ...
        foreach (var item in v)
        {
            if (!AllChars.specialChars.Contains(item) && !over.Any(d => d == item))// CAG.IsEqualToAnyElement(item, over))
            {
                sb.Append(item);
            }
        }
    ...
    {
        var p = SHSplit.SplitMore(line, SHData.ReturnCharsForSplitBySpaceAndPunctuationCharsAndWhiteSpaces(true).ToArray());
        if (p.Count < v)
        {
    ...
    public static bool HasTextRightFormat(string r, TextFormatDataString tfd)
    {
        ...
        while (true)
        {
            bool canBeAnyChar = actualFormatData.mustBe == null || actualFormatData.mustBe.Length == 0; //SunamoCollectionsShared.CA.IsEmptyOrNull();
            ...
            if (!isRightChar)
            {
                ...
                isRightChar = followingFormatData.mustBe.Any(d => d == r[actualChar]);  //CAG.IsEqualToAnyElement<char>(, );
                if (!isRightChar)
                {
                    return false;
                }
